...
const MAX_RETRIES = 3
const API_URL = 'https://steamcommunity.com/inventory'

Trade.prototype.getInventory = function getInventory(steamID64, appID, contextID, callback, retries) {
    request(`${API_URL}/${steamID64}/${appID}/${contextID}`, (error, response, body) => {
            ...
            if (descriptions && assets) {
                async.forEach(descriptions, (description, cbDesc) => async.forEach(assets, (asset, cbAsset) => {
                    if (
                        ...
                        function non_craftable() {
                            for (const i in description.descriptions) {
                                if (description.descriptions[i].value === '( Not Usable in Crafting )') {
                                    return true;
                                }
                            }
                            return false;
                        }
                        const name = non_craftable() ? 'Non-Craftable ' + description.market_hash_name : description.market_hash_name;
...
Trade.prototype.getItemType = function getItemType(marketHashName, type) {
    if (marketHashName.indexOf('Mann Co. Supply Crate Key') !== -1) {
        return { value: 0, name: 'key' }
    }
    if (marketHashName.indexOf('Strange') !== -1) {
        return { value: 1, name: 'strange' }
    }
    if (marketHashName.indexOf('Genuine') !== -1) {
        return { value: 2, name: 'genuine' }
    }
    if (marketHashName.indexOf('Vintage') !== -1) {
        return { value: 4, name: 'vintage' }
    }
