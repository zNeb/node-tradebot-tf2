    ...
    request(tfPrices, (error, response, body) => {
        ...
        if(error || response.statusCode !== 200) {
			...
			for (const i in items) {
                ...
                if(!(name.includes("Killstreak") || name.includes("(Factory New)") || name.includes("(Minimal Wear)") || name.includes("(Field-Tested)") || name.includes("(Well-Worn)") || name.includes("(Battle Scarred)"))) {
