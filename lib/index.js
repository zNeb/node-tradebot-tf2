                    ...
                    return async.forEach(Object.keys(botInventory), (index, cb) => {
                        ...
                        if (obj.bot.indexOf(item.assetid) !== -1) {
                            ...
                            for (const i in botInventory) {
                                if (userItem.includes(botInventory[i].data.market_hash_name) && botInventory[i].data.market_hash_name !== 'Mann Co. Supply Crate Key' && botInventory[i].data.market_hash_name !== 'Refined Metal' && botInventory[i].data.market_hash_name !== 'Reclaimed Metal' && botInventory[i].data.market_hash_name !== 'Scrap Metal') {
                                    return cb('You are trading an overstocked item!')
                                }
                            }
                        }
                        return cb()
